"""
Usage:
python eval_real_robot.py -i <ckpt_path> -o <save_dir> --robot_ip <ip_of_ur5>
...
# required 修改为default试试
# @click.option('--input', '-i', required=True, help='Path to checkpoint')            # 必需，帮助信息为“检查点的路径”
# @click.option('--output', '-o', required=True, help='Directory to save recording')  # 必需，帮助信息为“保存记录的目录”
# @click.option('--robot_ip', '-ri', required=True, help="UR5's IP address e.g. 192.168.0.204")   # 必需，帮助信息为“UR5的IP地址，例如 192.168.0.204”
...
@click.option('--output', '-o', required=True, help='Directory to save recording')  # 必需，帮助信息为“保存记录的目录”
@click.option('--robot_ip', '-ri', required=True, help="UR5's IP address e.g. 192.168.0.204")   # 必需，帮助信息为“UR5的IP地址，例如 192.168.0.204”
@click.option('--match_dataset', '-m', default=None, help='Dataset used to overlay and adjust initial condition')       # -match_dataset 或 -m，默认值为None，帮助信息为“用于覆盖和调整初始条件的数据集”
@click.option('--match_episode', '-me', default=None, type=int, help='Match specific episode from the match dataset')   # --match_episode 或 -me，默认值为None，类型为int，帮助信息为“匹配数据集中指定的集”
@click.option('--vis_camera_idx', default=0, type=int, help="Which RealSense camera to visualize.") # --vis_camera_idx，默认值为0，类型为int，帮助信息为“要可视化的RealSense相机”
@click.option('--init_joints', '-j', is_flag=True, default=False, help="Whether to initialize robot joint configuration in the beginning.") # --init_joints 或 -j，标志选项，默认值为False，帮助信息为“是否在开始时初始化机器人关节配置”
@click.option('--steps_per_inference', '-si', default=6, type=int, help="Action horizon for inference.")    # --steps_per_inference 或 -si，默认值为6，类型为int，帮助信息为“推理的动作视界”
@click.option('--max_duration', '-md', default=60, help='Max duration for each epoch in seconds.')          # --max_duration 或 -md，默认值为60，帮助信息为“每个周期的最大持续时间(秒)”
@click.option('--frequency', '-f', default=10, type=float, help="Control frequency in Hz.")                 # --frequency 或 -f，默认值为10，类型为float，帮助信息为“控制频率(Hz)”
@click.option('--command_latency', '-cl', default=0.01, type=float, help="Latency between receiving SapceMouse command to executing on Robot in Sec.") # --command_latency 或 -cl，默认值为0.01，类型为float，帮助信息为“从接收SpaceMouse命令到在机器人上执行的延迟(秒)”
def main(input, 
         output, 
         robot_ip, 
         match_dataset, 
