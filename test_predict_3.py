...
# 构建数据增强管道
_transform_list = [
    # RandomCrop（保持宽高比的随机裁剪）
    transforms.RandomResizedCrop(
        size=(int(240*0.95), int(320*0.95)),  # 基于95%比例计算
        scale=(0.95, 0.95),  # 固定裁剪比例
        ratio=(320/240, 320/240),  # 保持原始宽高比
...
obs_encoder = TimmObsEncoder(
    ...
    # 图像预处理配置
    transforms=transforms.Compose(_transform_list),
    downsample_ratio=32,
    use_group_norm=True,
    ...
    # 高级参数（来自代码默认值）
    in_channels=3,          # 输入通道数
    spatial_softmax=True,   # 是否使用空间softmax
    output_activation=None, # 输出激活函数
)
