...
from diffusion_policy.policy.diffusion_unet_timm_policy import DiffusionUnetTimmPolicy # 导入DiffusionUnetImagePolicy类
...
#     model_name=TransformerObsEncoder[vit_base_patch16_clip_224.openai],
#     pretrained=True,
#     frozen=False,
#     global_pool= '',
...
@hydra.main(
    version_base=None, # 关闭Hydra版本兼容性检查
    config_path=str(pathlib.Path(__file__).parent.joinpath(
        'diffusion_policy','config'))
)

# 输入参数：cfg 是通过Hydra解析后的配置对象
def main(cfg: OmegaConf):
    # resolve immediately so all the ${now:} resolvers
    # # will use the same time.
    # print(config_path)
    # 解析动态配置:确保所有配置参数在运行前完成解析
    OmegaConf.resolve(cfg)
    
    # cfg._target_ 必须指向一个继承自 BaseWorkspace 的类
    cls = hydra.utils.get_class(cfg._target_) # 通过类名获取类对象
    # workspace: BaseWorkspace = cls(cfg)     # 实例化工作区
    # workspace.run()                         # 执行训练流程  

    # 使用配置文件 --config-name=train_diffusion_unet_timm_umi_workspace 初始化模型
    model: DiffusionUnetTimmPolicy = hydra.utils.instantiate(cfg.policy)
    ...
    train_dataset = UmiDataset(**params)
    ...
    model.set_normalizer(train_dataset.get_normalizer())
    ...
    with torch.no_grad(), \
        ...
        for batch_idx, batch in enumerate(pbar):  # 遍历所有批次
            ...
            # obs_dict = batch[batch_idx]['obs']  # 获取观测字典
            ...
            print("Params Dict Keys:", model.normalizer.params_dict.keys())
